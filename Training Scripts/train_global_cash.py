...
from Networks.Global_networks import resnext50_32x4d, regularize_path_weights
import copy
...
def path_cleaning(global_path, info_df):
    cleaned_path = []
    seg_list = get_files(global_path)
...
if __name__ == '__main__':
    global_train= 'path/to/global_train_processed/'
    global_test= 'path/to/global_test_processed/'
    info_train = pd.read_csv('path/to/clinical_information/train.csv')
    info_val =  pd.read_csv('path/to/clinical_information/valid.csv')

    Train_list = path_cleaning(global_train,info_train)
    Val_list = path_cleaning(global_test,info_val)
    ...
    for epoch in tqdm(range(EPOCH)):
        ...
        with open('path/to/metric_global.txt', 'a') as f:
        ...
        save_path = 'path/to/metric_global/'
